...
import { PlusOutlined, EditOutlined, DeleteOutlined, UserOutlined, DollarOutlined, TrophyOutlined, HistoryOutlined, SearchOutlined } from '@ant-design/icons';
...
export default function ThuongComponent() {
    // Hooks
    const { danhSachLoaiTienThuong, updateLoaiTienThuong, deleteLoaiTienThuong, createLoaiTienThuong, getAllLoaiTienThuong } = useLoaiTienThuong()
    const { danhSachLichSuThuong, updateLichSuThuong, createLichSuThuong, deleteLichSuThuong } = useLichSuThuong()
    ...
    const dataSourceDanhSachLichSuThuong = danhSachLichSuThuong.map(dsltt => {
        ...
        const danhSachLoaiTienThuongFind = danhSachLoaiTienThuong.find(ltt => ltt.maLoaiTienThuong === dsltt.maLoaiTienThuong)
        return {
            ...dsltt,
    ...
    // Tính toán thống kê
    const tongSoNhanVienDuocThuong = new Set(danhSachLichSuThuong.map(item => item.maNhanVien)).size;
    const tongSoTienThuong = dataSourceDanhSachLichSuThuong.reduce((sum, item) => {
        // Nếu soTienThuongKhac có giá trị thì dùng nó
        if (item.soTienThuongKhac && item.soTienThuongKhac > 0) {
            return sum + item.soTienThuongKhac;
        } else {
            // Nếu không, lấy soTienThuong từ loại tiền thưởng
            const loaiTienThuong = danhSachLoaiTienThuong.find(ltt => ltt.maLoaiTienThuong === item.maLoaiTienThuong);
            return sum + (loaiTienThuong?.soTienThuong || 0);
        }
    }, 0);
    ...
    const getFilteredLichSu = () => {
        ...
        return dataSourceDanhSachLichSuThuong.filter(item =>
            ...
            getLoaiThuongName(item.maLoaiTienThuong).toLowerCase().includes(searchTextLichSu.toLowerCase()) ||
    ...
    const getFilteredLoaiThuong = () => {
        ...
        return danhSachLoaiTienThuong.filter(item =>
            item.tenLoaiTienThuong.toLowerCase().includes(searchTextLoaiThuong.toLowerCase()) ||
    ...
    const handleDeleteLoaiThuong = (maLoaiTienThuong) => {
        (dataSourceDanhSachLichSuThuong.filter(item => item.maLoaiTienThuong !== maLoaiTienThuong));
    ...
    const getLoaiThuongName = (maLoaiTienThuong) => {
        const loaiThuong = dataSourceDanhSachLichSuThuong.find(item => item.maLoaiTienThuong === maLoaiTienThuong);
            ...
            render: (_, record) => (
                <Space size="middle">
                    ...
                    <Popconfirm
                        ...
                        onConfirm={() => handleDeleteLichSuThuong(record.maNhanVien, record.maLoaiTienThuong)}
                        okText="Có"
                        cancelText="Không"
                    >
                        <Button
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                        ...
                        {modalType === 'lichsu' ? (
                            <>
                                ...
                                >
                                    <Select placeholder="Chọn loại thưởng">
                                        {danhSachLoaiTienThuong.map(item => (
                                            <Select.Option key={item.maLoaiTienThuong} value={item.maLoaiTienThuong}>
                                ...
                                <Form.Item
                                    name="lyDo"
                                    label="Lý do thưởng"
                                    rules={[{ required: true, message: 'Vui lòng nhập lý do thưởng!' }]}
                                >
                            ...
                            <>
                                <Form.Item
                                    ...
                                    rules={[{ required: true, message: 'Vui lòng nhập tên loại thưởng!' }]}
                                >
                ...
                >
                    ...
                    <Table
                        ...
                        dataSource={Array.from(new Set(dataSourceDanhSachLichSuThuong.map(item => item.maNhanVien)))
                            .map(maNV => {
