...
import { getCaLamDetailsByMaCaService, updateCaLamDetailsServices } from '../../services/calamServices';
...
export default function CaLamComponent() {
    ...
    const fetchAndSetShiftDetails = useCallback(async (maCa) => {
        setLoadingDetails(true);
        try {
            const details = await getCaLamDetailsByMaCaService(maCa);
            setShiftDetailsByDay(details);
        } catch (error) {
            console.error("Lỗi khi tải chi tiết ca làm:", error);
            apiNotification.error("Không thể tải chi tiết ca làm. Vui lòng thử lại.");
    ...
    const handleSaveDailyShiftDetails = async (maCa, updatedDetails) => {
        try {
            await updateCaLamDetailsServices(maCa, updatedDetails);
            ...
            apiNotification.error("Lưu chi tiết ca làm thất bại: " + (error.response?.data?.message || error.message));
            throw error;
        }
    };
    ...
    const handleCellClickForDetail = useCallback(async (record, e) => {
    ...
    const mainTableColumns = [
        ...
        },
        {
            ...
            fixed: 'right',
            render: (_, record) => (
                <Space onClick={(e) => e.stopPropagation()}> {/* RẤT QUAN TRỌNG: Ngăn chặn click vào Space này mở modal chi tiết */}
                    <Button
                        type="text"
                        icon={<EditOutlined />}
                        onClick={(e) => {
    ...
    return (
        <div style={{ padding: '24px', background: 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)', minHeight: '100vh' }}>
            <div style={{ margin: '0 auto' }}>
                ...
                {/* Modal for Adding/Editing Shift Name */}
                <Modal
                    centered
                    size='large'
                    title={
