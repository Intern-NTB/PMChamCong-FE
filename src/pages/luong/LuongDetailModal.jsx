...
export default function LuongDetailModal({
  ...
  const handleExportExcel = () => {
    exportToExcel([record], selectedMonthYear ? selectedMonthYear.format("MM/YYYY") : "", record.phongBan, true);
  };

  const handleExportPDF = () => {
    generatePDF([record], selectedMonthYear ? selectedMonthYear.format("MM/YYYY") : "", true);
  };

  return (
    <Modal
      visible={visible}
      title={`Bảng lương tháng ${selectedMonthYear ? selectedMonthYear.format("MM/YYYY") : ""} - ${record.hoTen}`}
      onCancel={onCancel}
      footer={null}
      width={700}
    >
      <Descriptions column={1} bordered size="middle">
        <Descriptions.Item label={<><CalendarOutlined /> Tháng/Năm</>}>
      ...
      <Space style={{ marginTop: 20, justifyContent: "center", width: "100%" }}>
        <Button icon={<FileExcelOutlined />} onClick={handleExportExcel} type="primary" size="middle">
          Xuất Excel
        </Button>
        <Button icon={<FilePdfOutlined />} onClick={handleExportPDF} type="default" size="middle">
          Xuất PDF
        </Button>
      </Space>
    </Modal>
