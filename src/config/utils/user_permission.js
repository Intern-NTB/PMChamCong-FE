...
export const NghiPhepPermissions = {
    ...
    canEdit: () => isAdmin() || checkPermissionByKey('nghiphep:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('nghiphep:delete') || isHRManager(),
    canApprove: () => isAdmin() || checkPermissionByKey('nghiphep:approve') || isHRManager(),
...
export const PhanQuyenPermissions = {
    ...
    canManageRoles: () => isAdmin() || checkPermissionByKey('phanquyen:manage_roles'),
};

// Example: System Settings
export const HeThongPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('hethong:view') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('hethong:edit'),
};

// Example: Department Management
export const PhongBanPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('phongban:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('phongban:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('phongban:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('phongban:delete') || isHRManager(),
};

// Example: Account Management
export const TaiKhoanPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('taikhoan:view') || isHRManager(),
    canAdd: () => isAdmin() || checkPermissionByKey('taikhoan:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('taikhoan:edit'),
    canDelete: () => isAdmin() || checkPermissionByKey('taikhoan:delete'),
    canResetPassword: () => isAdmin() || checkPermissionByKey('taikhoan:reset_password'),
};

// Example: Shift Management
export const CaLamPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('calam:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('calam:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('calam:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('calam:delete') || isHRManager(),
};

// Example: Priority Object Management
export const DoiTuongUuTienPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('doituonguutien:view') || isHRManager(),
    canAdd: () => isAdmin() || checkPermissionByKey('doituonguutien:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('doituonguutien:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('doituonguutien:delete') || isHRManager(),
};

// Example: Role Management
export const VaiTroPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('vaitro:view') || isHRManager(),
    canAdd: () => isAdmin() || checkPermissionByKey('vaitro:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('vaitro:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('vaitro:delete') || isHRManager(),
};

// Example: Holiday Management
export const NgayLePermissions = {
    canView: () => isAdmin() || checkPermissionByKey('ngayle:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('ngayle:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('ngayle:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('ngayle:delete') || isHRManager(),
};

// Example: Bonus Management
export const ThuongPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('thuong:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('thuong:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('thuong:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('thuong:delete') || isHRManager(),
};

// Example: Penalty Management
export const PhatPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('phat:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('phat:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('phat:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('phat:delete') || isHRManager(),
};

// Example: Department History
export const LichSuPhongBanPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('lichsuphongban:view') || isHRManager(),
};

// Example: Salary History
export const LichSuLuongPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('lichsuluong:view') || isHRManager(),
};

// Example: Employee Management
export const NhanVienPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('nhanvien:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('nhanvien:create') || isHRManager(),
    canEdit: () => isAdmin() || checkPermissionByKey('nhanvien:edit') || isHRManager(),
    canDelete: () => isAdmin() || checkPermissionByKey('nhanvien:delete'),
};

// Example: Timekeeping Management
export const ChamCongPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('chamcong:view') || isHRManager() || isHRAssistant(),
    canAdd: () => isAdmin() || checkPermissionByKey('chamcong:create') || isHRAssistant(),
    canEdit: () => isAdmin() || checkPermissionByKey('chamcong:edit') || isHRAssistant(),
    canDelete: () => isAdmin() || checkPermissionByKey('chamcong:delete') || isHRAssistant(),
};

// Example: Report Management
export const BaoCaoPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('baocao:view') || isHRManager(),
    canGenerate: () => isAdmin() || checkPermissionByKey('baocao:generate') || isHRManager(),
};

// Example: Time Clock Device Management
export const MayChamCongPermissions = {
    canView: () => isAdmin() || checkPermissionByKey('maychamcong:view') || isHRManager(),
...
export const hasPermission = (moduleName, action) => {
    ...
    if (!modulePermissions) {
        console.warn(`Permissions for module "${moduleName}" are not defined in user_permission.js.`);
        return false;
    }

    const actionMethodName = `can${action.charAt(0).toUpperCase() + action.slice(1)}`;

    if (typeof modulePermissions[actionMethodName] === 'function') {
        return modulePermissions[actionMethodName]();
    } else {
        console.warn(`Action "${action}" not defined for module "${moduleName}" in user_permission.js.`);
        return false;
    }
};
